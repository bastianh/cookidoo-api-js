...
This is an early, incremental port. It currently covers the OAuth2/PKCE login flow, access-token refresh, token persistence, `getUserInfo`, the shopping list (recipes, ingredient items, additional items, clearing the list), and recipes (search, full details). Further endpoints (custom recipes, calendar, collections, device/remote-monitoring, ...) are ported incrementally from the [Python client](https://github.com/miaucl/cookidoo-api).
