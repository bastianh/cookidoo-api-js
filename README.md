...
This is an early, incremental port. It currently covers the OAuth2/PKCE login flow, access-token refresh, token persistence, `getUserInfo`, the shopping list (recipes, ingredient items, additional items, clearing the list), recipes (search, full details), custom recipes (get, list, add-from, remove), the calendar (get a week, add/remove recipes, including custom ones), collections (managed and custom: count/get/add/remove, plus add/remove-recipe on custom collections), and the REST side of devices/remote-monitoring (paired appliances, monitorable device ids, push-token register/unregister, push-payload decoding). Actually *receiving* Firebase push messages needs a real FCM client and is out of scope -- see the [`cookidoo-api-js` README](packages/cookidoo-api-js/README.md#usage) for details. Further endpoints are ported incrementally from the [Python client](https://github.com/miaucl/cookidoo-api).
